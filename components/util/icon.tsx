...
const iconSizeClass: { [name: string]: string } = {
...
export const Icon = ({ data, className = "", tinaField = "" }: IconProps) => {
  ...
  const iconSizeClasses = size
    ? iconSizeClass[size]
    : iconSizeClass[Object.keys(iconSizeClass)[size]];

  const iconColor: string =
    (color ? (color === "primary" ? theme : color) : theme) || "light";
  ...
  if (style == "circle") {
    ...
    const iconColorClasses =
      iconColorClass[
        iconColor === theme || iconColor === "primary" ? "white" : iconColor
      ].regular;
    return (
      <IconSVG
        data-tina-field={tinaField}
